...
export default function AccountView({ onBackAction, onLogoutAction }: AccountViewProps) {
  ...
  return (
    <div className="fixed inset-0 bg-white z-[70] flex flex-col font-sora">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 py-8 flex flex-col items-center bg-white">
          <div className="relative mb-4 cursor-pointer group" onClick={() => setShowAvatarPicker(true)}>
            <div className="w-20 h-20 rounded-full bg-white border-4 border-slate-50 shadow-xl overflow-hidden group-hover:opacity-90 transition-opacity">
              <img
        ...
        <div className="px-6 py-2 space-y-8">
          {sections.map((section, idx) => (
            <div key={idx} className="space-y-3">
              <h3 className="text-sm font-bold text-slate-400 ml-1">
                {section.title}
              </h3>
          ...
          <div className="flex flex-col items-center py-6 mb-10">
            <button
              ...
              className="text-red-500 font-bold text-xs hover:text-red-600 transition-colors border-b border-red-200 pb-0.5"
            >
              Delete Account
            </button>
          </div>
        ...
        {showAvatarPicker && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[32px] z-[110] px-6 pt-2 pb-10 shadow-2xl overflow-hidden max-h-[70vh] flex flex-col"
            >
              {/* Handle */}
              ...
              <button
                onClick={() => setShowAvatarPicker(false)}
                className="mt-6 w-full bg-[#003da5] hover:bg-[#003da5]/90 text-white font-bold py-2.5 rounded-xl shadow-lg transition-all active:scale-[0.98]"
              >
                Done
              </button>
            </motion.div>
